...
export function MapaSucursales({ sucursales, onSelect, selectedSucursalId }: MapaSucursalesProps) {
    ...
    sucursales.forEach(sucursal => {
      ...
      const icon = {
        url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
          sucursal.tipo === 'hogar'
            ? `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="${sucursal.estado === 'activo' ? '#2563eb' : '#6b7280'}" stroke="${sucursal.estado === 'activo' ? '#ffffff' : '#e5e7eb'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>`
            : `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="${sucursal.estado === 'activo' ? '#ea580c' : '#6b7280'}" stroke="${sucursal.estado === 'activo' ? '#ffffff' : '#e5e7eb'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="5" width="16" height="16" rx="2"></rect><path d="M4 9h16"></path><path d="M9 21V9"></path></svg>`
        )}`,
      ...
      const marker = new google.maps.Marker({
        ...
        icon,
        animation: selectedSucursalId === sucursal.id 
      ...
      const tooltipContent = `
        <div class="p-3 max-w-xs">
          ...
          ${sucursal.cliente ? `<div class="text-xs text-gray-600 mb-2">Cliente: ${sucursal.cliente}</div>` : ''}
  ...
  }, [map, infoWindow, sucursales, onSelect, selectedSucursalId, markerClusterer]);
      ...
      {!mapLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-800 z-10">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 dark:text-gray-400">Cargando mapa...</p>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 z-10">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-gray-600 dark:text-gray-400">Sucursal Hogar</span>
          </div>
          <div className="flex items-center space-x-2">
            <Building className="text-orange-600 dark:text-orange-400" size={16} />
            <span className="text-xs text-gray-600 dark:text-gray-400">Sucursal Cliente</span>
          </div>
        </div>
      </div>
    </div>
